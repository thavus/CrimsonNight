...
Imported.SeanBattleOverwrite = true;

var Sean = Sean || {};
Sean.detect = Sean.detect || {};
...
var tmp = PluginManager.parameters('SeanBattleOverwrite')["LOS Blocking Terrain"].split(",");
Sean.detect.detectTerrain = [];

if (tmp && tmp[0]) {
	for (var i = 0; i < tmp.length; i++) {
		Sean.detect.detectTerrain.push(Number(tmp[i]));
	};
};

// Blocking regions array
tmp = PluginManager.parameters('SeanBattleOverwrite')["LOS Blocking Regions"].split(",");
Sean.detect.bRegions = [];
if (tmp || tmp[0]) {
	for (var i = 0; i < tmp.length; i++) {
		Sean.detect.bRegions.push(Number(tmp[i]));
	};
};

// tile size
Sean.detect.tile = Number(PluginManager.parameters('SeanBattleOverwrite')["Tile Size"]);
Sean.detect.searches = Number(PluginManager.parameters('SeanBattleOverwrite')["Search Limit"]);

// Behaviors
Sean.detect.behaviors = {};
var i = 0;
do {
    tmp = PluginManager.parameters('SeanBattleOverwrite')["Behavior " + i];
	if (tmp) {
		Sean.detect.behaviors[i] = tmp.split(",");
		for (var i2 = 2; i2 < Sean.detect.behaviors[i].length; i2++) {
			Sean.detect.behaviors[i][i2] = Number(Sean.detect.behaviors[i][i2]);
		};
	};
    i++;
}
...
Sean.detect.event = function(id,dist,los) {
	if ($gameSystem._undetectable) return false;
	return $gameMap.event(id).distDetect(dist,los);
};

Sean.detect.dist = function(x1,y1,x2,y2) {
	return Math.sqrt(Math.pow(x1 - x2,2) + Math.pow(y1 - y2,2));
};

Sean.detect.isBlock = function(x,y) {
	var x = Math.round(x);
	var y = Math.round(y);
	if (Sean.detect.bRegions.contains($gameMap.regionId(x,y))) return true;
	if (Sean.detect.detectTerrain.contains($gameMap.terrainTag(x,y))) return true;
	// Blocking event
...
Sean.detect.los = function(char1,char2) {
	...
	if (Math.abs(a.x - b.x) >= Math.abs(a.y - b.y)) {
		...
		for (var x = a.x; x <= b.x; x++) {
			var y = slope * x + int;
			if (Sean.detect.isBlock(x,y)) return false;
		}
...
Sean.detect.Game_Character_initMembers = Game_Character.prototype.initMembers;
Game_Character.prototype.initMembers = function() {
	this.setDetectVars();
	Sean.detect.Game_Character_initMembers.call(this);
};

Game_Character.prototype.distDetect = function(range,los,id,balloon) {
	var balloon = balloon || 0;
	var id = id || 0;
	var range = range * Sean.detect.tile;
	...
	var dist = Sean.detect.dist(x1,y1,x2,y2);

	if (dist <= range) { // If in radius range of target
		if ((los && Sean.detect.los(target,this)) || !los) {  // If LOS to target is not blocked
...
Game_Character.prototype.detector = function(id) {
	...
	this._detectBalloon = Sean.detect.behaviors[id][6];
	var detected = this.distDetect(Sean.detect.behaviors[id][2],Sean.detect.behaviors[id][3]);
...
Game_Character.prototype.doDetectMove = function(id,type) {
	...
	if (type == 0) {
		...
		this._moveSpeed = Sean.detect.behaviors[id][4];
		this._moveFrequency = Sean.detect.behaviors[id][5];
	};
	
	
	switch (Sean.detect.behaviors[id][type]) {
		case 'approach':
...
Sean.detect.Game_Event_setupPageSettings = Game_Event.prototype.setupPageSettings;
//Game_Map.prototype.distDetect = Game_Character.prototype.distDetect;
//Game_Event.prototype.distDetect = Game_Character.prototype.distDetect;

Game_Event.prototype.setupPageSettings = function() {
	Sean.detect.Game_Event_setupPageSettings.call(this);
	this.setDetectStuff();
};
