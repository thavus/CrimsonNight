...
Window_StatusInfo.prototype.setRateColor = function(rate, symb) {
    var colorId = 0;
    if (rate >= 3.0) {
...
Window_StatusInfo.prototype.drawStatesInfo = function() {
    ...
    for (var i = 0; i < maxCols; ++i) {
      for (var j = 0; j < infoArray[i].length; ++j) {
	    ...
	    for(var k = 0; k < this._actor._states.length; k++){
	    	if(this._actor._states[k] == stateId){
...
Window_StatusInfo.prototype.drawStatesData = function(stateId, dx, dy, dw) {
    ...
    console.log(this.systemColor());
    for(var i = 0; i < $dataStates[stateId].traits.length; i++){
    	var code = $dataStates[stateId].traits[i].code;
    	var id = $dataStates[stateId].traits[i].dataId;
    	...
    	// experience
    	if(code == 23 && id == 9){ 
    		stateTxt += 'Exp:' + symb + val + '% ';
    	}
    	// max mp
    	if(code == 21 && id == 1){ 
    		stateTxt += 'MaxMP:' + symb + val + '% ';
    	}
    	// max hp
    	if(code == 21 && id == 0){ 
    		stateTxt += 'MaxHP:' + symb + val + '% ';
    	}
    	// Bleeding/poison
    	if(code == 22 && id == 7){ 
    ...
    if (this._actor.isStateResist(stateId)) stateTxt = 0;
    dx += this.textPadding();
    dw -= this.textPadding() * 2;
    this._bypassResetTextColor = true;
    this.changeTextColor(this.systemColor());
    ...
    this._bypassResetTextColor = false;
    this.setRateColor(stateTxt, symb);
    var text = stateTxt;
    this.drawText(text, dx, dy, dw, 'right');
};
